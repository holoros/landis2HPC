...
### Phase 2, real precomputed data wiring (next, no new infrastructure)
Replace the embedded sample and the illustrative envelopes with the real per plot trajectories already produced on Cardinal. The per state atlas JSON (`perseus/dashboard/atlas/{ST}.json`) and the harvested production vectors feed actual calibrated trajectories. When the MN, WI, and MI chains land and the factorial sweep runs, the climate by harvest cells become real instead of envelopes. Still a static site, just backed by richer JSON or a small set of parquet files read in the browser.
