...
### Phase 3, live runs through a backend (seed shipped and wired)
A thin API service (FastAPI) sits between the GUI and Cardinal. The GUI posts a scenario config; the service translates it into the existing pipeline (`build_plot_scenario_{ST}.sh`, `cma_es_optimize_{ST}.py`, the factorial scenario builder), submits the SLURM jobs, polls `squeue`, and reads the result back. The GUI polls a job id and renders results when ready. As of 2026-05-21 the seed exists at `perseus/backend/` (with CORS enabled) and the GUI is wired to it: the selected-plot card has a "Live run on Cardinal" panel that takes a backend URL and a plot id, submits the current scenario, polls, and overlays the returned trajectory as a "Live LANDIS run" line. The single-plot path is live; statewide and factorial runs reuse the same submission layer. Remaining for production: authentication on OSC accounts, a job database instead of the in-memory registry, a results cache, and a deployment (OSC OnDemand interactive app, a UMaine or CRSF virtual machine with key based SSH to Cardinal, or a small container). This is what turns the scenario builder from a what if envelope into an actual run.
