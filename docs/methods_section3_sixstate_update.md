...
Minnesota, Wisconsin, and Michigan fill in the intermediate Great Lakes regime: calibration cuts the year-100 carbon by 1.5 to 1.7 fold (literature 131, 177, 179 Mg C per hectare; calibrated 78, 118, 105), a strict between-extremes result. Across all five states (Fig. statewide_carbon_5state), the year-100 ratio of calibrated to literature carbon matches each state's median ANPP multiplier to within 0.05: Washington 0.33 versus theta 0.30, Minnesota 0.60 versus 0.60, Wisconsin 0.67 versus 0.65, Michigan 0.59 versus 0.63, and Maine 1.32 versus 1.31. The per-species corrections propagate cleanly to the state-aggregate carbon, which is the strongest internal-consistency check we have: the small-scale multiplier and the large-scale carbon trajectory carry the same information. All five trajectories come from real 100-year LANDIS runs over the state plot sets (n in 400 to 4429), not from the modeled envelope.
