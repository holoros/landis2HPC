...
Done in v0.2: a SQLite job store so submitted jobs survive a restart (`db.py`, path via
`PERSEUS_API_DB`), and optional API-key auth on the action endpoint. Set `PERSEUS_API_KEY`
to require an `X-API-Key` header; leave it unset for local development. The GUI live-run
panel has an optional API key field that it sends as `X-API-Key`. `/health` reports whether
auth is open or required.

Still to add: OSC-account auth (the API key is a single shared secret, fine for a controlled
deployment but not per user); a results cache keyed by scenario hash so identical requests do
not resubmit; rate limiting and a per-user job cap; plot-id validation against the state
inventory; and paramiko with a connection pool instead of shelling out to ssh per call.
