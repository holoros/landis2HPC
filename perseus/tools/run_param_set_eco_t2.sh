#!/bin/bash
# run_param_set_eco_t2.sh
#
# Tier 1.5 (per-ecoregion) evaluator that mirrors run_param_set_t2.sh:
#   - Uses apply_theta_eco.py to write the modified SppEcoregionData CSV
#   - Replaces the broken osgeo aggregator with gdalinfo CLI
#   - Cleans up its runs/ directory after per_plot.csv is written
#
# Usage on Cardinal:
#   bash run_param_set_eco_t2.sh <THETA_ECO_CSV> <THETA_TAG>
#
# Output:
...
if [ "$#" -lt 2 ]; then
  echo "Usage: bash run_param_set_eco_t2.sh <THETA_ECO_CSV> <THETA_TAG>"
  exit 1
fi
...
mkdir -p $BAY/runs
LOG=$BAY/launch.log
echo "=== run_param_set_eco_t2 $TAG started $(date) ===" > $LOG

##############################################################################
# 1. Apply theta_eco to baseline SppEcoregionData
##############################################################################
