#!/bin/bash
# build_plot_scenario_IN.sh
#
# Build a single-plot LANDIS-II scenario directory for an IN PERSEUS plot.
# 1-cell landscape with FIA-derived IC, restricted to the plot's EPA L3
# ecoregion. PRISM_IN_l3.csv is already in wide format (Year,Month,Variable,
# <eco_cols>) so the climate.csv is just a column subset.
#
# Usage: bash build_plot_scenario_IN.sh <PLOT_ID> <baseline> <none|perseus>

set -euo pipefail

if [ "$#" -lt 3 ]; then
  echo "Usage: bash build_plot_scenario_IN.sh <PLOT_ID> <baseline> <none|perseus>"
  exit 1
fi
...
PLOT_ECO_CSV=$TOOLS/plot_to_ecoregion_IN.csv
PRISM_WIDE=$INPUTS/PRISM_IN_l3.csv
...
# IN EPA L3 ecoregions present in the FIA sample: 54-56, 71-72
case "$RAW_ECO" in
  54) ECO=54; ECO_NAME='"Central Corn Belt Plains"' ;;
  55) ECO=55; ECO_NAME='"Eastern Corn Belt Plains"' ;;
  56) ECO=56; ECO_NAME='"Southern Michigan Northern Indiana Drift Plains"' ;;
  71) ECO=71; ECO_NAME='"Interior Plateau"' ;;
  72) ECO=72; ECO_NAME='"Interior River Valleys and Hills"' ;;
...
echo "Built IN scenario at $PLOT_DIR (eco $ECO from raw $RAW_ECO, PRISM col $ECO_COL)"
