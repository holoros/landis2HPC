#!/bin/bash
# run_param_set_OH_t2.sh — inner CMA-ES loop for OH Tier 2.
# Apply per-species theta -> run stratified plot subset -> compute LL -> write file.
# Usage: bash run_param_set_OH_t2.sh <THETA_CSV> <TAG>
...
CHAIN=${TAG%%_iter*}              # oh_t2_v1_iter0_cand0 -> oh_t2_v1
BAY=$PERSEUS/bayesian/$CHAIN/$TAG
INPUTS=$STATE/inputs
PLOT_SUBSET=$PERSEUS/oh_t2_plotsubset.txt

mkdir -p $BAY
LOG=$BAY/launch.log
echo "=== run_param_set_OH_t2 $TAG started $(date) ===" > $LOG

# Build stratified subset if missing — 100 plots per ecoregion
if [ ! -f "$PLOT_SUBSET" ]; then
  PLOT_ECO=$TOOLS/plot_to_ecoregion_OH.csv
...
python3 $TOOLS/apply_theta_OH_perspecies.py --theta-csv "$THETA_CSV" --baseline-spp "$SPP_BASE" --out "$SPP_MOD" >> $LOG 2>&1
...
#!/bin/bash
#SBATCH --job-name=oht2_${TAG}_c${c}
...
bash $TOOLS/build_plot_scenario_OH.sh \$PLOT baseline none > /dev/null
...
# Compute LL via multi-cycle FIA pairing (OH)
python3 - <<PY >> $LOG 2>&1
import csv, glob, os, math
with open("$TOOLS/untreated_plots_OH.csv") as f:
...
rm -rf $BAY/runs
echo "=== run_param_set_OH_t2 $TAG done $(date) ===" >> $LOG
