#!/usr/bin/env python3
"""apply_theta_IN_perspecies.py — write theta-scaled SppEcoregionData.csv for Indiana.
...
def read_theta(path):
    ...
    with open(path) as f:
        ...
        if reader.fieldnames and "param" in reader.fieldnames and "value" in reader.fieldnames:
            ...
            for sp, v in zip(SPECIES_IN, vals[:N_SPECIES]):
                mult[f"ANPP_{sp}"] = v
            for sp, v in zip(SPECIES_IN, vals[N_SPECIES:]):
                mult[f"BMAX_{sp}"] = v
    return mult
...
def main():
    ...
    expected = {f"ANPP_{s}" for s in SPECIES_IN} | {f"BMAX_{s}" for s in SPECIES_IN}
