#!/bin/bash
# run_param_set_MI_t2.sh — inner CMA-ES loop for MN Tier 2.
# Apply per-species theta -> run stratified plot subset -> compute LL -> write file.
# Usage: bash run_param_set_MI_t2.sh <THETA_CSV> <TAG>
...
PLOT_SUBSET=$PERSEUS/mi_t2_plotsubset.txt

mkdir -p $BAY
LOG=$BAY/launch.log
echo "=== run_param_set_MI_t2 $TAG started $(date) ===" > $LOG

# Build stratified subset if missing — 100 plots per ecoregion
if [ ! -f "$PLOT_SUBSET" ]; then
  PLOT_ECO=$TOOLS/plot_to_ecoregion_MI.csv
...
#!/bin/bash
#SBATCH --job-name=mit2_${TAG}_c${c}
...
bash $TOOLS/build_plot_scenario_MI.sh \$PLOT baseline none > /dev/null
...
with open("$TOOLS/untreated_plots_MI.csv") as f:
...
rm -rf $BAY/runs
echo "=== run_param_set_MI_t2 $TAG done $(date) ===" >> $LOG
