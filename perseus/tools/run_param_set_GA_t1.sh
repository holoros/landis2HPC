#!/bin/bash
# run_param_set_GA_t1.sh — Tier 1 uniform-theta evaluation for GA (with scratch cleanup)
# Usage: bash run_param_set_GA_t1.sh <THETA> <TAG>
set -uo pipefail
...
mkdir -p $BAY/runs
LOG=$BAY/launch.log
echo "=== run_param_set_GA_t1 $TAG theta=$THETA started $(date) ===" > $LOG

# 1. Apply uniform theta
...
for IC_DIR in $ICS/plot_*; do
  ...
  [ -f "$IC" ] && [ "$(wc -l < $IC)" -gt 1 ] || { SKIPPED=$((SKIPPED+1)); continue; }
...
# 6. Scratch cleanup — preserve per_plot.csv + LL, drop runs/ to free space
