#!/bin/bash
# run_param_set_IN_t2.sh — inner CMA-ES loop for IN Tier 2.
# Apply per-species theta -> run stratified plot subset -> compute LL -> write file.
# Usage: bash run_param_set_IN_t2.sh <THETA_CSV> <TAG>
...
TOOLS=$LANDIS/tools
STATE=$LANDIS/states/IN
PERSEUS=$STATE/perseus
CHAIN=${TAG%%_iter*}              # in_t2_v1_iter0_cand0 -> in_t2_v1
BAY=$PERSEUS/bayesian/$CHAIN/$TAG
INPUTS=$STATE/inputs
PLOT_SUBSET=$PERSEUS/in_t2_plotsubset.txt

mkdir -p $BAY
LOG=$BAY/launch.log
echo "=== run_param_set_IN_t2 $TAG started $(date) ===" > $LOG

# Build stratified subset if missing — 100 plots per ecoregion
if [ ! -f "$PLOT_SUBSET" ]; then
  PLOT_ECO=$TOOLS/plot_to_ecoregion_IN.csv
  awk -F',' 'NR>1 && $3+0>0 {print $3,$1}' $PLOT_ECO | sort -n | \
...
python3 $TOOLS/apply_theta_IN_perspecies.py --theta-csv "$THETA_CSV" --baseline-spp "$SPP_BASE" --out "$SPP_MOD" >> $LOG 2>&1
...
#!/bin/bash
#SBATCH --job-name=int2_${TAG}_c${c}
...
bash $TOOLS/build_plot_scenario_IN.sh \$PLOT baseline none > /dev/null
...
# Compute LL via multi-cycle FIA pairing (IN)
python3 - <<PY >> $LOG 2>&1
import csv, glob, os, math
with open("$TOOLS/untreated_plots_IN.csv") as f:
...
for traj_f in glob.glob("$BAY/runs/plot_*/biomass_trajectory.csv"):
    ...
    pys = sorted(pred)  # DENSE_PAIRING: interpolate to every observed year in window
...
rm -rf $BAY/runs
echo "=== run_param_set_IN_t2 $TAG done $(date) ===" >> $LOG
