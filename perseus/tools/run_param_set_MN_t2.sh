#!/bin/bash
# run_param_set_MN_t2.sh — inner CMA-ES loop for MN Tier 2.
# Apply per-species theta -> run stratified plot subset -> compute LL -> write file.
# Usage: bash run_param_set_MN_t2.sh <THETA_CSV> <TAG>
...
#!/bin/bash
#SBATCH --job-name=mnt2_${TAG}_c${c}
...
bash $TOOLS/build_plot_scenario_MN.sh \$PLOT baseline none > /dev/null
...
with open("$TOOLS/untreated_plots_MN.csv") as f:
...
rm -rf $BAY/runs
echo "=== run_param_set_MN_t2 $TAG done $(date) ===" >> $LOG
