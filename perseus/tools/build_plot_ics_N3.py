...
The map and its congener lumping were derived empirically from the live-tree SPCD
histograms of IN_TREE.csv and OH_TREE.csv (STATUSCD==1) on 2026-05-31 (see
docs/conus_ecoregion_clusters.md / state_templates/README.md). The 23 modeled N3 species
are mapped 1:1 where a direct FIA code exists; minor congeners are lumped to the nearest
modeled species exactly as the MN builder lumps (silver/boxelder maple -> RM; green ash
-> WAS; cottonwood/bigtooth aspen -> QA; slippery elm -> AE; pignut/bitternut/generic
hickory -> MOK_HK; chestnut/chinkapin oak -> WO; blackjack oak -> BO; Shumard -> NRO).
Species outside the 23-species N3 pool (black cherry 762, eastern redcedar 68, black
locust 901, hackberry 461/462, hophornbeam 701, other pines) are intentionally dropped,
the same modeled-species-only convention used for every other PERSEUS state.

LANDIS N3 species (23): WO NRO BO POST SHO SHA_HK MOK_HK SM RM YP BE WAS BAS BSW WALN
SWEETGUM BLACKGUM SYC SASSAFRAS DOGWOOD EWP QA AE.
"""
...
SPCD_TO_LANDIS = {
    ...
    # --- maples ---
    318: "SM",                              # sugar maple
    316: "RM", 317: "RM", 313: "RM",        # red/silver/boxelder -> red maple group
    # --- hickories ---
    ...
    # --- aspen + poplar ---
    746: "QA", 743: "QA", 742: "QA", 741: "QA",  # quaking/bigtooth/cottonwood/balsam poplar
    # --- 1:1 ---
